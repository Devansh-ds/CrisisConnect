...
function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-1000 w-full border-b border-transparent bg-gradient-to-r from-blue-600 to-indigo-600">
        ...
        <div className="h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 ml-4">
              <Link to="/" className={`${linkBase} ${location.pathname === "/" ? active : inactive}`}>Dashboard</Link>
              <Link to="/zones" className={`${linkBase} ${location.pathname.startsWith("/zones") ? active : inactive}`}>Disaster Zones</Link>
              <Link to="/sos" className={`${linkBase} ${location.pathname.startsWith("/sos") ? active : inactive}`}>SOS Requests</Link>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            >
              <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-white text-blue-700 text-xs font-semibold">A</span>
